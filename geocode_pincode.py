...
def main():
    addresses = pd.read_csv('pincode_data.csv')
    print addresses.shape
    # print addresses[:5]
    addresses['cleaned_address'] = addresses.pincode.astype(str) + ', ' + addresses.city.str.lower()
    addresses[['standard_address', 'latitude', 'longitude']
              ] = addresses.cleaned_address.apply(getLatLng)
    addresses.to_csv('pincode_data_v2.csv', index=False)
...
def getLatLng(address):
    try:
        ...
        if loc:
            return pd.Series([loc.address.encode('utf-8', 'ignore'), loc.latitude, loc.longitude],
